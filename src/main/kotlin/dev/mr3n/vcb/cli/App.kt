...
fun main(args: Array<String>) {
    ...
    val onlineMode by parser.option(ArgType.Boolean, "online_mode", "om", "Mojangでプレイヤーを認証する必要があるかどうか").default(true)
    val preventClientProxyConnections by parser.option(ArgType.Boolean, "prevent_client_proxy_connections", "pcpc", "クライアントのISP/ASが、Mojangの認証サーバからのものと異なる場合プレイヤーはKickされます。これは一部のVPNやプロキシ接続を切断するものですが保護としては脆弱です。").default(false)
    ...
    val announceForge by parser.option(ArgType.Boolean, "announce_forge", "af", "もしあなたのネットワークが`一貫して一つのmodpackを動かしている`のであれば、代わりに ping-passthrough = \"mods\" を使って、サーバリストでよりきれいに表示することを検討してみてください。").default(false)
    val kickExistingPlayers by parser.option(ArgType.Boolean, "kick_existing_players", "kep", "trueの場合、重複した接続を試みようとしたプレイヤー(既にサーバーに居る方)をKickします。").default(false)
    ...
    val haproxyProtocol by parser.option(ArgType.Boolean,"haproxy_protocol", "hp", "HAProxyのProxyProtocolと互換性を持たせることができます。").default(false)
    val tcpFastOpen by parser.option(ArgType.Boolean,"tcp_fast_open", "tfo", "VelocityでTCPファストオープンのサポートを有効にします。Linuxでのみ動作します。").default(false)
    val bungeePluginMessageChannel by parser.option(ArgType.Boolean, "bungee_plugin_message", "bpm", "Velocity で BungeeCord プラグインのメッセージングチャネルをサポートするかどうか").default(true)
    val showPingRequests by parser.option(ArgType.Boolean, "show_ping_requests", "spr", "クライアントからVelocityへの ping リクエストをすべてログに記録するかどうか").default(false)
    val failoverOnUnexpectedServerDisconnect by parser.option(ArgType.Boolean, "failover_on_unexpected_server_disconnect", "fousd", "読み取りタイムアウトの場合を除き、ユーザーをフォールバックさせようとすることで、明示的な切断メッセージなしでユーザーが予期せずサーバーへの接続を失った状況を適切に処理するかどうか").default(true)
    val announceProxyCommands by parser.option(ArgType.Boolean, "announce_proxy_commands", "apc", "プロキシ コマンドを 1.13 以降のクライアントに定義するかどうか").default(true)
    val logCommandExecutions by parser.option(ArgType.Boolean, "log_command_executions", "lce", "すべてのコマンドの実行をログに記録するかどうか").default(false)
    val queryEnabled by parser.option(ArgType.Boolean, "query_enabled", "qe", "GameSpy 4のクエリ応答への応答を有効にするかどうか。").default(true)
